...
void Dealer::loopDeal(Players &player,Judge &judge,int* cards) {
    bool choice=0; 
    bool loopEnd=false;
...
//                cout<<loopSize<<" inside loop."<<endl; 
                ...
                hand[1][(player.getCount()+loopSize)-1]=cards[dealt];
                cards[dealt]=0; 
                dealt++;  
                choice=1;
                break; 
    ...
    //Dealer hit or stay
        if(loopEnd==false) {             // validate the choice by player
            ...
            if(sums[0]<17) {
                ...
                while(cards[dealt]==0) dealt++;     // make sure the card being dealt isn't one that's already been used
                hand[0][(hitCount+loopSize)-1]=cards[dealt];   // give a new card
        ...
        loopEnd=false; // picked an invalid choice input, let them try again
    }
}
