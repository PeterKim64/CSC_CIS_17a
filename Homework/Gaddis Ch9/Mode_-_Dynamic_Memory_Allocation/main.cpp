 ...
 * Author: Peter Kim 
 * Created: Oct 1, 2023
 * Purpose:  Find Mode Set, Frequency of Mode, and number of modes
 */
...
int *mode(const int *array,int arySize){
    ...
    for(int i = 0; i < arySize; i++){
        ...
        //find appearances for each number
        for(int j = i; j < arySize; j++){
            //compare to see if it appears more than once
            if(ary[i] == ary[j]) {
                freq += 1;
            }
        }

        //See which is max frequency and the number of modes
        if(freq >= max && freq != 1) {
            max = freq;
            modeAry[count + 2] = count;
            count += 1;
        }
    }
